...
def create_nqueens_csp(n = 8):
    ...
    for i in domain:
        csp.add_variable(('Q', i), domain)
    for i in range(n):
        for j in domain[i+1:]:
            l, r = domain[i], j
            csp.add_binary_factor(('Q',l),('Q',r),lambda x,y:x!=y)
            csp.add_binary_factor(('Q',l),('Q',r),lambda x,y:abs(l-r)!=abs(x-y))
    # END_YOUR_ANSWER
    return csp
        ...
        if not self.mcv:
            ...
            for var in self.csp.variables:
                if var in assignment:
                    continue
                l=len([val for val in self.domains[var] if self.get_delta_weight(assignment, var, val)>0])
                p.append((var, l))
            return min(p, key = lambda x: x[1])[0]
            # END_YOUR_ANSWER
                ...
                for val2 in self.domains[var2]:
                    ...
                    for val1 in self.domains[var1]:
                        if self.csp.binaryFactors[var1][var2][val1][val2] > 0:
                            domain_.append(val2)
                            break
                if len(domain_) != len(self.domains[var2]):
                    self.domains[var2]=domain_
                    q.append(var2)
        # END_YOUR_ANSWER
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    local_domain = [i for i in range(maxSum+1)]
    aux_domain = [(i, j) for i in local_domain for j in local_domain]

    result = ('sum', name, 'aggregated')
    csp.add_variable(result, local_domain)
    
    if len(variables) == 0:
        csp.add_unary_factor(result, lambda val: val == maxSum)
        return result
    ...
    for i, X_i in enumerate(variables):
        ...
        if i > 0:
            csp.add_binary_factor(A_i, ('sum', name, i-1), lambda a, b: a[0] == b[1])
...
def create_lightbulb_csp(buttonSets, numButtons):
    ...
    def get_aux_variable(csp, name, variables):
        result = ('aux', name, 'aggregated')
        csp.add_variable(result, [True, False])

        if len(variables) == 0:
            csp.add_unary_factor(result, lambda r: r == False)
            return result
        
        for i, X_i in enumerate(variables):
            A_i = ('aux', name, i)
            csp.add_variable(A_i, [(False, False), (False, True), (True, False), (True, True)])
            if i == 0: csp.add_unary_factor(A_i, lambda a: a[0] == False)
            else: csp.add_binary_factor(A_i, ('aux', name, i-1), lambda a, b: a[0] == b[1])
    ...
    for button in range(numButtons):
        csp.add_variable('B' + str(button), [True, False])
    for i, buttons in enumerate(buttonSets):
        bulb = get_aux_variable(csp, 'L' + str(i), ['B' + str(button) for button in buttons])
        csp.add_unary_factor(bulb, lambda x: x == True)
    # END_YOUR_ANSWER
    return csp
